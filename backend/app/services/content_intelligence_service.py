...
class Entity:
    ...
    def __init__(self, text: str, entity_type: str, confidence: float = 0.0):
        ...
        self.relationships: list["Entity"] = []
        self.first_seen = datetime.utcnow()
        self.last_seen = datetime.utcnow()
        self.mention_count = 1
        self.source_urls: set[str] = set()

    def add_mention(self, source_url: Optional[str] = None) -> None:
    ...
    def add_relationship(self, relationship: str, target_entity: str) -> None:
...
class Relationship:
    ...
    def __init__(
        self, source: str, target: str, relationship_type: str, confidence: float = 0.5
    ):
        self.source = source
        self.target = target
    ...
    def add_mention(self, context: Optional[str] = None) -> None:
...
class ContentIntelligenceService:
    ...
    def __init__(self) -> None:
        ...
        self.entities: Dict[str, Any] = {}  # text -> Entity
        self.relationships: List[Any] = []  # List of Relationship objects
    ...
    def _setup_entity_patterns(self) -> None:
        ...
        self.relationship_patterns = [
            (
                r"(\w+(?:\s+\w+)*)\s+(?:is|was|are|were)\s+(?:the|a|an)?\s*(CEO|president|director|manager|founder|owner|chair|chairperson|head|chief|leader)\s+of\s+(\w+(?:\s+\w+)*)",
                "LEADS_ORGANIZATION",
            ),
            (
                r"(\w+(?:\s+\w+)*)\s+(?:works?|worked?)\s+(?:at|for|in)\s+(\w+(?:\s+\w+)*)",
                "WORKS_AT",
            ),
            (
                r"(\w+(?:\s+\w+)*)\s+(?:is|was)\s+(?:born|located|based)\s+in\s+(\w+(?:\s+\w+)*)",
                "LOCATED_IN",
            ),
            (r"(\w+(?:\s+\w+)*)\s+(?:owns?|owned?)\s+(\w+(?:\s+\w+)*)", "OWNS"),
            (
                r"(\w+(?:\s+\w+)*)\s+(?:partners?|partnered?)\s+with\s+(\w+(?:\s+\w+)*)",
                "PARTNERS_WITH",
            ),
            (
                r"(\w+(?:\s+\w+)*)\s+(?:acquires?|acquired?)\s+(\w+(?:\s+\w+)*)",
                "ACQUIRES",
            ),
            (
                r"(\w+(?:\s+\w+)*)\s+(?:invests?|invested?)\s+in\s+(\w+(?:\s+\w+)*)",
                "INVESTS_IN",
            ),
        ]
    ...
    async def extract_relationships(
        ...
        for pattern, rel_type in self.relationship_patterns:
            ...
            for match in matches:
                ...
                if (
                    source_text.lower() in entity_texts
                    and target_text.lower() in entity_texts
                ):
                    ...
                    relationship = Relationship(
                        source_text, target_text, rel_type, confidence
                    )
        ...
        if settings.OPENROUTER_API_KEY:
            try:
                llm_relationships = await self._extract_relationships_with_llm(
                    content, entities
                )
    ...
    async def analyze_temporal_patterns(
        ...
        temporal_analysis: Dict[str, Any] = {
            "dates_mentioned": [],  # type: ignore[assignment]
            "time_periods": [],  # type: ignore[assignment]
    ...
    async def extract_intelligence_summary(
        self, content: str, source_url: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Extracting intelligence summary from {source_url or 'unknown source'}"
        )
        ...
        logger.info(
            f"Generated intelligence summary with {len(entities)} entities and {len(relationships)} relationships"
        )
        return summary
    ...
    async def _extract_entities_with_llm(
        ...
        try:
            ...
            response = await self.llm_service.analyze_content(prompt, max_tokens=1000)  # type: ignore[attr-defined]
    ...
    def _calculate_intelligence_scores(
        self, content: str, entities: List[Entity], relationships: List[Relationship]
    ) -> Dict[str, float]:
    ...
    async def _extract_key_insights(
        ...
        if high_conf_entities:
            insights.append(
                f"High-confidence entities identified: {', '.join([e.text for e in high_conf_entities[:3]])}"
            )
        ...
        if strong_relationships:
            insights.append(
                f"Strong relationships found: {len(strong_relationships)} connections identified"
            )
        ...
        if len(org_entities) > 1:
            insights.append(
                f"Multiple organizations mentioned: {', '.join([e.text for e in org_entities[:3]])}"
            )
        ...
        if len(location_entities) > 1:
            insights.append(
                f"Geographic scope: {len(location_entities)} locations referenced"
            )
        ...
        if settings.OPENROUTER_API_KEY and len(content) > 200:
            try:
                ...
                response = await self.llm_service.analyze_content(
...
async def analyze_content_intelligence(
    content: str, source_url: Optional[str] = None
) -> Dict[str, Any]:
    """
