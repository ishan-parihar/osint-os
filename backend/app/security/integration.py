...
def get_cors_origins(environment: str) -> list:
    ...
    if environment == "development":
        ...
        staging_url = os.getenv(
            "STAGING_FRONTEND_URL", "https://staging.scrapecraft.com"
        )
...
def create_redis_client() -> redis.Redis:
    ...
    try:
        ...
        if os.getenv("ENVIRONMENT", "development").lower() == "development":
            logger.warning(
                "Continuing without Redis - rate limiting and caching disabled"
            )
            return None
        else:
            raise RuntimeError(
                "Redis connection required for production security features"
            )
...
def initialize_security_components() -> bool:
    ...
    try:
        ...
        if redis_client:
            ...
            logger.error(
                "Redis client not available - security components not fully initialized"
            )
            return False
...
def validate_security_environment() -> list:
    ...
    if environment == "production":
        ...
        if len(jwt_secret) < 64:
            messages.append(
                "WARNING: JWT_SECRET should be at least 64 characters for production"
            )
...
async def setup_application_security(app):
    ...
    try:
        ...
        features = {
            ...
            "Rate Limiting": security_config.RATE_LIMIT_ENABLED
            if security_config
            else False,
            "Audit Logging": security_config.AUDIT_LOG_ENABLED
            if security_config
            else False,
        ...
        if jwt_secret:
            ...
            if len(jwt_secret) < 64:
                health_status["recommendations"].append(
                    "Use longer JWT secret (64+ characters)"
                )
        else:
            health_status["checks"]["jwt_secret"] = {
                "status": "fail",
