...
class Investigation(Base):
    ...
    # Basic fields - uuid is inherited from Base
    title: Mapped[str] = mapped_column(String, nullable=False)
    description: Mapped[str] = mapped_column(Text, nullable=False)

    # Classification and priority
    classification: Mapped[InvestigationClassification] = mapped_column(
        Enum(InvestigationClassification), nullable=False
    )
    priority: Mapped[InvestigationPriority] = mapped_column(
        Enum(InvestigationPriority), nullable=False
    )
    status: Mapped[InvestigationStatus] = mapped_column(
        Enum(InvestigationStatus), nullable=False
    )

    # Workflow
    current_phase: Mapped[InvestigationPhase] = mapped_column(
        Enum(InvestigationPhase), nullable=False
    )
    ...
    intelligence_requirements = relationship(
        "IntelligenceRequirement",
        back_populates="investigation",
        cascade="all, delete-orphan",
    )
    agent_assignments = relationship(
        "AgentAssignment", back_populates="investigation", cascade="all, delete-orphan"
    )
    collected_evidence = relationship(
        "CollectedEvidence",
        back_populates="investigation",
        cascade="all, delete-orphan",
    )
    analysis_results = relationship(
        "AnalysisResult", back_populates="investigation", cascade="all, delete-orphan"
    )
    threat_assessments = relationship(
        "ThreatAssessment", back_populates="investigation", cascade="all, delete-orphan"
    )
    phase_transitions = relationship(
        "PhaseTransition", back_populates="investigation", cascade="all, delete-orphan"
    )
...
class InvestigationTarget(Base):
    ...
    # uuid is inherited from Base
    investigation_uuid: Mapped[str] = mapped_column(
        String, ForeignKey("investigations.uuid"), nullable=False
    )

    type: Mapped[InvestigationTargetType] = mapped_column(
        Enum(InvestigationTargetType), nullable=False
    )
    ...
    collection_requirements: Mapped[List[str]] = mapped_column(
...
class IntelligenceRequirement(Base):
    ...
    investigation = relationship(
        "Investigation", back_populates="intelligence_requirements"
    )
...
class AgentAssignment(Base):
    ...
    assigned_targets: Mapped[List[str]] = mapped_column(ARRAY(String), nullable=True)
    current_task: Mapped[Dict[str, Any]] = mapped_column(JSON, nullable=True)
    status: Mapped[AgentStatus] = mapped_column(Enum(AgentStatus), nullable=False)
    performance_metrics: Mapped[Dict[str, Any]] = mapped_column(JSON, nullable=True)
...
class CollectedEvidence(Base):
    ...
    content_tags: Mapped[List[str]] = mapped_column(ARRAY(String), nullable=True)

    # Metadata
    evidence_metadata: Mapped[Dict[str, Any]] = mapped_column(JSON, nullable=True)
...
class AnalysisResult(Base):
    ...
    evidence_uuid: Mapped[str] = mapped_column(
        String, ForeignKey("collected_evidence.uuid"), nullable=True
    )
...
class ThreatAssessment(Base):
    ...
    targets: Mapped[List[str]] = mapped_column(ARRAY(String), nullable=True)
...
class PhaseTransition(Base):
    ...
    from_phase: Mapped[InvestigationPhase] = mapped_column(
        Enum(InvestigationPhase), nullable=False
    )
    to_phase: Mapped[InvestigationPhase] = mapped_column(
        Enum(InvestigationPhase), nullable=False
    )
...
class InvestigationReport(Base):
    ...
    recipients: Mapped[List[str]] = mapped_column(ARRAY(String), nullable=True)
...
class FinalAssessment(Base):
    ...
    key_findings: Mapped[List[str]] = mapped_column(ARRAY(String), nullable=True)
    confidence_level: Mapped[float] = mapped_column(Float, nullable=False)
    overall_threat_level: Mapped[ThreatLevel] = mapped_column(
        Enum(ThreatLevel), nullable=False
    )
    classification: Mapped[InvestigationClassification] = mapped_column(
        Enum(InvestigationClassification), nullable=False
    )
