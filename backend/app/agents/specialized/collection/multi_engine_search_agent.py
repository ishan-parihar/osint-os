...
class MultiEngineSearchAgent(OSINTAgent):
    ...
    def _get_system_prompt(self) -> str:
        ...
        """
        return """You are a Multi-Engine Search Specialist, an expert OSINT agent that conducts comprehensive searches across multiple search engines simultaneously.

Your primary responsibilities:
1. Execute searches across multiple search engines (DuckDuckGo, Brave, StartPage, Qwant)
2. Aggregate and deduplicate results from different sources
3. Analyze result quality and relevance
4. Categorize findings by content type and relevance
5. Extract key insights and patterns from search results
...
Maintain high standards of accuracy and relevance in all search operations."""

    def _process_output(
        self, raw_output: str, intermediate_steps: Optional[List] = None
    ) -> Dict[str, Any]:
        """
        Process and structure the raw output from the agent.
        """
        # Since this agent uses the MultiSearchEngine directly, the output is already structured
        ...
        try:
            ...
            if isinstance(raw_output, str):
                ...
                try:
                    ...
                    return {
                        "search_response": raw_output,
                        "processed_at": datetime.utcnow().isoformat(),
                        "agent_type": "multi_engine_search",
                    }
            else:
                return raw_output
        except Exception:
            return {
                "search_response": str(raw_output),
                "processed_at": datetime.utcnow().isoformat(),
                "agent_type": "multi_engine_search",
            }
    ...
    def _enhance_search_results(
        ...
        enhanced["categorized_results"] = self._categorize_results(
            search_results.get("results", [])
        )

        # Add key insights
        enhanced["key_insights"] = self._extract_key_insights(
            search_results.get("results", []), query
        )
    ...
    def _analyze_query_complexity(self, query: str) -> str:
        ...
        complex_count = sum(
            1 for indicator in complex_indicators if indicator in query_lower
        )
    ...
    def _identify_content_types(self, results: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        for result in results:
            ...
            if any(
                ...
                for indicator in ["news", "article", "report"]
            ):
                content_types["news"] += 1
            elif any(
                indicator in url
                for indicator in [".edu", "scholar", "academic", "research"]
            ):
                content_types["academic"] += 1
            elif any(
                indicator in url for indicator in [".com", "shop", "store", "buy"]
            ):
                content_types["commercial"] += 1
            elif any(
                indicator in url or indicator in title
                for indicator in ["blog", "post", "medium"]
            ):
    ...
    def _extract_key_insights(
        ...
        # General insights
        insights.append(
            f"Found {len(results)} relevant results across multiple search engines"
        )

        # Top domains insight
        top_domains = self._extract_top_domains(results)
        if top_domains:
            domain_names = [str(d["domain"]) for d in top_domains[:3]]
            insights.append(f"Top sources include: {', '.join(domain_names)}")
        ...
        if dominant_type[1] > 0:
            insights.append(
                f"Predominant content type: {dominant_type[0]} ({dominant_type[1]} results)"
            )
