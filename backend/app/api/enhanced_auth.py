...
from ..services.rbac import require_permission, Permission
from .common import (
    APIResponse,
    ErrorCode,
    ValidationError,
...
class APIKeyRequest(BaseModel):
    ...
    google_search_key: Optional[str] = Field(None, description="Google Search API key")
    bing_key: Optional[str] = Field(None, description="Bing Search API key")
...
async def register(request: Request, user_data: RegisterRequest) -> APIResponse:
    ...
    try:
        ...
        if not await auth_service.check_rate_limit(
            key=user_data.username, endpoint_type="register", ip_address=ip_address
        ):
            raise HTTPException(
        ...
        return create_success_response(
            data=user_response.dict(), message="User registered successfully"
        )
...
async def login(request: Request, login_data: LoginRequest) -> APIResponse:
    ...
    try:
        ...
        logger.info(
            f"User logged in: {user.username} (role: {user.role.value}) from {ip_address}"
        )

        return create_success_response(
            data=token_data.dict(), message="Login successful"
        )
...
async def get_current_user_info(
    ...
    try:
        ...
        return create_success_response(
            data=user_response.dict(), message="User information retrieved successfully"
        )
...
async def refresh_access_token(request: Request, refresh_token: str) -> APIResponse:
    """
    ...
    try:
        ...
        # Check rate limiting
        if not await auth_service.check_rate_limit(
            key=username, endpoint_type="token_refresh", ip_address=ip_address
        ):
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
        ...
        return create_success_response(
            data=token_data.dict(), message="Token refreshed successfully"
        )
...
async def get_user_permissions(
    ...
    try:
        ...
        return create_success_response(
            data=permission_data, message="Permissions retrieved successfully"
        )
    ...
    try:
        ...
        if security_config:
            InputValidator.validate_password(
                password_data.new_password, security_config
            )
        ...
        success = await auth_service.update_user(
            ...
            {"hashed_password": new_hashed_password, "updated_at": datetime.now()},
        )
        ...
        ip_address = request.client.host if request.client else "unknown"
        logger.info(
            f"Password changed for user: {current_user.username} from {ip_address}"
        )
...
async def save_api_keys(
    ...
    try:
        ...
        api_keys_data = {
            ...
            "username": current_user.username,
            "role": current_user.role,
            "permissions": [],  # Will be populated from RBAC system
        ...
        ip_address = request.client.host if request.client else "unknown"
        logger.info(
            f"API keys saved securely for user: {current_user.username} from {ip_address}"
        )

        return create_success_response(message="API keys saved successfully")
...
async def get_api_keys(
    ...
    try:
        ...
        api_keys_file = (
            Path("secure_storage") / f"enhanced_api_keys_{current_user.username}.json"
        )
        ...
        if api_keys_file.exists():
            try:
                ...
                api_keys = {
                    ...
                    "scrapegraph_key": mask_key(
                        api_keys_data.get("scrapegraph_key", "")
                    ),
                    "google_search_key": mask_key(
                        api_keys_data.get("google_search_key", "")
                    ),
                ...
                logger.warning(
                    f"Failed to read API keys for {current_user.username}: {e}"
                )
        ...
        return create_success_response(
            data=api_keys, message="API keys retrieved successfully"
        )
...
# Note: Admin user management endpoints have been moved to admin.py module
...
async def lock_user_account(
    ...
    try:
        ...
        return create_success_response(message=f"User {username} has been locked")
...
async def unlock_user_account(
    ...
    try:
        ...
        return create_success_response(message=f"User {username} has been unlocked")
